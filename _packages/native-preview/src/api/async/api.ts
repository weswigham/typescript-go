...
export class API<FromLSP extends boolean = false> {
    ...
    async batchRequests(requests: RequestObjects[]): Promise<{responses: ResponseObjects[]}> {
        const resp = await this.client.apiRequest("batchRequests", { requests: requests });
        ...
        return resp as {responses: ResponseObjects[]};
    }
